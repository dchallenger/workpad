...
class Shift_class extends MY_PrivateController
{
	public function __construct()
	{
		$this->load->model('shift_class_model', 'mod');
		parent::__construct();
	}
	...
	public function index(){   
		...
		$shifts = $this->db->get_where( 'time_shift', array( 'deleted' => 0 ) )->result_array();
		$data['shifts'] = $shifts;
     ...
     public function get_list()
    {
        ...
        $page = ($page < 0 ? 0 : $page);
        $shiftClasses = $this->input->post('filter');
        $shifts = $this->input->post('filter_by');
        $records = $this->mod->_get_list($page, 10, $search, '', $trash, $shiftClasses, $shifts);
