...
class Erequest_admin extends MY_PrivateController
{
    ...
    public function index()
    {
        ...
        $data['erequest_admin'] = isset($this->permission['list']) ? $this->permission['list'] : 0;

        $this->load->model('erequest_model', 'erequest_ad');
        $data['erequest'] = isset($permission[$this->erequest_ad->mod_code]['list']) ? $permission[$this->erequest_ad->mod_code]['list'] : 0;
	...
	function save($child_call = false)
	{
		...
		$uploads = $_POST['resources_request_upload_hr'];
		
		if(isset($_POST['resources_request']['notify_immediate'])){
			$users_data = $this->db->get_where( 'users_profile', array( 'user_id' => $_POST['resources_request']['user_id'] ) )->row_array();	
			$_POST['resources_request']['notify_immediate'] = $users_data['reports_to_id'];	
		}else{
			$_POST['resources_request']['notify_immediate'] = 0;
		}

		if(isset($_POST['resources_request']['notify_others'])){
			$_POST['resources_request']['notify_others'] = implode(',', $_POST['resources_request']['notify_others']);
		}else{
			$_POST['resources_request']['notify_others'] = null;
		}
		unset($_POST['resources_request_upload']);
		unset($_POST['resources_request_upload_hr']);
		unset($_POST['request_status_id']);
		unset($_POST['resources_request']['notify_others-temp']);
		// $_POST['resources_request']['user_id'] = $this->user->user_id;

		//closed if cancelled
		if($_POST['resources_request']['request_status_id'] == 5){
			$saved_message = 'returned';
		}else{
		...
		// 	echo "<pre>";
		// 	print_r($_POST);
		// 	// print_r($users_data['reports_to_id']);
		// exit();
		parent::save( true );
		...
		if( $this->response->saved )
		{
			...
			if( count($uploads['upload_id']) > 0 )
			{
				...
				foreach( $uploads as $upload_id )
				{
					...
					$this->db->insert('resources_request_upload_hr', $insert);
				}
			}
			if($_POST['resources_request']['request_status_id'] == 4){				
				$set_approver_status = "UPDATE {$this->db->dbprefix}resources_request_approver 
										SET request_status_id = 4 
										WHERE request_id = {$this->record_id} 
										AND user_id = {$this->user->user_id} 
										 ";										
				$result_update = $this->db->query( $set_approver_status );
				// mysqli_next_result($this->db->conn_id);
				$email_erequest = $this->mod->email_erequest($this->record_id);
			}
		}
