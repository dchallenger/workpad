	...
	<ul class="page-breadcrumb breadcrumb">
		@section('page-breadcrumb-back')
			<li class="pull-right btn-group" style="position:relative;">
				<a href="{{ get_mod_route('performance_master') }}"><button class="btn blue" type="button">
				<span>Back</span>
				</button></a>
			</li>
		@show
		...
		<li>
			<a href="{{ get_mod_route('performance_master') }}">{{ lang('performance.performance_master') }}</a>
			<i class="fa fa-angle-right"></i>
		</li>
		<li>
	...
	<div class="col-md-9">
		<div class="portlet">
			<div class="portlet-body">
				<div >
					<ul class="nav nav-tabs ">
						@if ($allow_scorecard)
						<li class=""><a href="{{ get_mod_route('scorecard') }}">Balance Scorecard</a></li>
						@endif
						@if ($allow_library)
						<li class=""><a href="{{ get_mod_route('library') }}">Library</a></li>
						@endif
						@if ($allow_rating_scale)
						<li class="hidden"><a href="{{ get_mod_route('rating_group') }}">Rating Scale</a></li>
						@endif
						@if ($allow_performance)
						<li class="active"><a href="{{ get_mod_route('performance') }}">Performance Type</a></li>
						@endif
						@if ($allow_notification)
						<li class="hidden"><a href="{{ get_mod_route('performance_notification') }}">Notification</a></li>
						@endif
					</ul>
				</div>
			</div>
			...
			<div class="portlet-title">
				<div class="caption"><i class="fa {{ $mod->icon}}"></i>List of Performances</div>
