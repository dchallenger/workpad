		...
		<div class="form-group">
			...
			<div class="col-md-7">							
				<div class="make-switch" data-on-label="&nbsp;{{ lang('common.option_yes') }}&nbsp;" data-off-label="&nbsp;{{ lang('common.option_no') }}&nbsp;">
						    	<input type="checkbox" value="1" @if( $record['users_sbu_unit.status_id'] ) checked="checked" @endif name="users_sbu_unit[status_id][temp]" id="users_sbu_unit-status_id-temp" class="dontserializeme toggle"/>
						    	<input type="hidden" name="users_sbu_unit[status_id]" id="users_sbu_unit-status_id" value="@if( $record['users_sbu_unit.status_id'] ) 1 @else 0 @endif"/>
				</div> 				
			</div>	
		</div>					
	</div>
