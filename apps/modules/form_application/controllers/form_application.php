...
class Form_application extends MY_PrivateController
{
    ...
    function add( $record_id = '', $child_call = false ){
        $type = $record_id; //change to this to fit with upgrade of higher version of php, it should be the same with the parent class
        ...
        if( $type == "" ){
            ...
            foreach($disable_no_credit as $disable){
                ...
                if(sizeof($balance_data) > 0){
                    ...
                    foreach($balance_data as $balance){
                        if(isset($no_credit_disable[$disable['form_id']])) {
            ...
            if( $record_id ){
                ...
                if($form_info['form_id']  == 10){
                    ...
                    if (isset($data['shift_details']['logs_time_in'])) {
            ...
            switch( $form_info['form_id'] ){
                case get_time_form_id('CWS'):
                   $data['shift_id']['val'] = isset($data['shift_details']['shift_id']) ? $data['shift_details']['shift_id'] : '';
                break;
        ...
        if( $record_id ){
            ...
            if( $record_id ){
                ...
                if($data['ut_type']  == 0){
                    ...
                    $data['ut_time_in_out'] = ($logs_out == "-") ? date("M d, Y - h:i A", strtotime(isset($data['shift_details']['shift_time_end']) ? $data['shift_details']['shift_time_end'] : '00:00:00')) : date("M d, Y - h:i A", strtotime($logs_out));
                }else{
                    $logs_in = (isset($data['shift_details']['logs_time_in']) ? $data['shift_details']['logs_time_in'] : '-');
                    $data['ut_time_in_out'] = ($logs_in == "-") ? date("M d, Y - h:i A", strtotime(isset($data['shift_details']['shift_time_start']) ? $data['shift_details']['shift_time_start'] : '00:00:00')) : date("M d, Y - h:i A", strtotime($logs_in));
                }
            }
...
/*            $data['ut_type'] = $this->mod->check_ut_type($record_id);
    ...
    {   
        ...
        switch ($data['form_status_id']['val']) {
            case 6:
                $label = 'Date Approved';
                $date_adc = ($forms_info['user_id'] != 0 ? $forms_info['date_approved'] : $forms_info['created_on']);
                break;
            case 7:
    ...
    function get_shift_details()
    {
        ...
        if($this->input->post('form_type') == 10){ //undertime/excused tardiness form
            if($this->input->post('utype') == 0){
                ...
                $shift_details['ut_time_in_out'] = ($logs_in == "-") ? date("M d, Y - h:i A", strtotime(isset($shift_details['shift_time_start']) ? $shift_details['DATE'] .' '. $shift_details['shift_time_start'] : '00:00:00')) : date("h:i A", strtotime($logs_in));
            }
        }
