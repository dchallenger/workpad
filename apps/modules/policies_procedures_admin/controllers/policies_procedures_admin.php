...
class Policies_procedures_admin extends MY_PrivateController
{
	public function __construct()
	{
		$this->load->model('policies_procedures_admin_model', 'mod');
		parent::__construct();
	}
    ...
    public function index()
    {
        ...
        $data['policies_admin'] = isset($this->permission['list']) ? $this->permission['list'] : 0;

        $this->load->model('policies_procedures_model', 'policies_mod');
        $data['policies'] = isset($permission[$this->policies_mod->mod_code]['list']) ? $permission[$this->policies_mod->mod_code]['list'] : 0;
	...
	function single_upload()
	{
		...
		if( isset($file->error) && $file->error != "" )
		{
			...
			if (strpos($file->error, 'upload_max_filesize') !== false){
				$file->error = 'The uploaded file exceeds the maximum file size to upload';
			}	
