...
class Recruitform extends MY_publicController
{
   ...
   //      	}
	  ...
	  //       	}
			...
			if (isset($post['recruitment_personal_history'])) {
				$partners_personal = $post['recruitment_personal_history'];
				if (!isset($partners_personal['education-year-from'])) {
					$this->response->message[] = array(
			...
			//Character Reference tab
				...
				if (isset($post['recruitment_personal_history'])) {
					$partners_personal = $post['recruitment_personal_history'];
					if (isset($partners_personal['reference-name'])) {
						$validation_rules[] = 
						array(
	...
	function save($child_call = false){
        ...
        /***** START Form Validation (hard coded) *****/
		//table assignment (manual saving)
		$partners_personal = array_merge($_POST['recruitment_personal'],$this->mod->get_partners_personal($this->user->user_id)); //get partner personal record base on login id;
		$partners_personal_history = $this->mod->get_partners_personal_history($this->user->user_id); //get partner personal record base on login id;
		$users_profile = $this->mod->get_users_profile($this->user->user_id); //get partner personal record base on login id;
		$post['recruitment'] = array_merge($_POST['recruitment'],$users_profile[0]);
		...
		// validate if already apply for the same position
		$the_same_position = $this->mod->check_the_same_position($this->user->user_id,$post['recruitment']['oth_position'],$partners_personal['position_sought']);
		if ($the_same_position == 2) {
			$this->response->message[] = array(
				'message' => 'You have pending application for this position',
				'type' => 'error'
				);

			$this->_ajax_return();
		}
		if ($the_same_position == 1) {
			$this->response->message[] = array(
				'message' => 'You have pending application for this other position',
				'type' => 'error'
				);  			
	...
	function save_abraham($child_call = false){
