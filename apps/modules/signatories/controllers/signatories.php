...
class Signatories extends MY_PrivateController
{
	...
	function delete_signatory()
	{
		...
		if($this->response->message[0]['type'] == 'success'){
			$data['class_id'] = (property_exists($approver_class_details, 'company_id')) ? $approver_class_details->class_id : 0 ;
			$data['company_id'] = (property_exists($approver_class_details, 'company_id')) ? $approver_class_details->company_id : 0 ;
			$data['department_id'] = (property_exists($approver_class_details, 'department_id')) ? $approver_class_details->department_id : 0 ;
			$data['position_id'] = (property_exists($approver_class_details, 'position_id')) ? $approver_class_details->position_id : 0 ;
			$data['user_id'] = (property_exists($approver_class_details, 'user_id')) ? $approver_class_details->user_id : 0 ;
			...
			// {
				...
				switch($data['class_id']){
					...
					$update_pending_approver = $this->mod->update_existing_performance_approvers($data['class_id'], $data['user_id'], $data['position_id'], $data['department_id'], $data['company_id']);
					break;
	...
	function save_signatory()
	{
		...
		if(empty($id))
		{
			...
			if (isset($data['user_id']))
				$data['employee'] = $this->input->post('user_id');			
		}
		else{
			$previous_main_data = $this->db->get_where($table, array('id' => $id))->row_array();
			...
			if ($previous_main_data['approver_id'] == $data['approver_id']) {
				unset($previous_main_data['alias']);
				unset($data['alias']);
			}
		}
		

		$this->mod->audit_logs($this->user->user_id, $this->mod->mod_code, $this->response->action, $table, $previous_main_data, $data);
 ...
 // die;	
	...
	function count_affected_forms(){
		...
		$approver_class_details = new stdClass();
		$set_for = $this->input->post('set_for');
		...
		$data['class_id'] = (property_exists($approver_class_details, 'class_id')) ? $approver_class_details->class_id : $class_id ;
		$data['company_id'] = (property_exists($approver_class_details, 'company_id')) ? $approver_class_details->company_id : $company_id ;
		$data['department_id'] = (property_exists($approver_class_details, 'department_id')) ? $approver_class_details->department_id : $department_id ;
		$data['position_id'] = (property_exists($approver_class_details, 'position_id')) ? $approver_class_details->position_id : $position_id ;
		$data['user_id'] = (property_exists($approver_class_details, 'user_id')) ? $approver_class_details->user_id : $user_id ;
		...
		switch($data['class_id']){
			...
			$this->response->pending_forms_count = $this->mod->select_existing_pending_forms($data['class_id'], $data['user_id'], $data['position_id'], $data['department_id'], $data['company_id']);
			break;
			case 19: //performance
			//select existing performance planning/appraisal approver
			$this->response->performance = 1;
			$this->response->pending_performance_count = $this->mod->select_existing_pending_performance($data['class_id'], $data['user_id'], $data['position_id'], $data['department_id'], $data['company_id']);
			break;
			...
			case 42://Mobile Application Plan	
			$this->response->map = 1;
			$this->response->pending_map_count = $this->mod->select_existing_pending_partner_loan_application($data['class_id'], $data['position_id'], $data['department_id'], $data['company_id'],  $data['user_id']);
			break;
			case 43://Omnibus Loan Applicaiton
			$this->response->ola = 1;
			$this->response->pending_ola_count = $this->mod->select_existing_pending_partner_loan_application($data['class_id'], $data['position_id'], $data['department_id'], $data['company_id'],  $data['user_id']);
			break;
			case 44://Car Plan Loan Application	
