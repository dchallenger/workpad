											...
											@if(isset($template_section) && $template_section->num_rows() > 0)
												@foreach($template_section->result_array() as $key1 => $val1)
													@if($val1['parent_id'] > 0 &&  $val1['section_type_id'] != 9)
														...
														<input type="hidden" name="section_coach_section_rating[{{$val1['template_section_id']}}][]" class="section_coach_section_rating_{{$val1['template_section_id']}}">
														...
														<input type="hidden" name="section_coach_total_weighted_score[{{$val1['template_section_id']}}][]" class="section_coach_total_weighted_score_{{$val1['template_section_id']}}">
														@if($val1['is_core'] == 0)
															<tr>
															...
															@if(isset($library_competencies) && !empty($library_competencies))
																@foreach($library_competencies[$val1['library_id']] as $key2 => $val2)
																	<?php
																		...
																		$core_score_car_library_coach_rating = (isset($appraisal_applicable_score_library_ratings[$val1['template_section_id']][$val['scorecard_id']]) ? $appraisal_applicable_score_library_ratings[$val1['template_section_id']][$val['scorecard_id']]['coach_rating'] : '');
																	?>																																
																	...
																	<input type="hidden" name="core_score_car_library_total_weighted_score[{{$val1['template_section_id']}}][{{$val2->library_value_id}}][]" class="core_score_car_library_total_weighted_score_{{$val2->library_value_id}}">
																	<input type="hidden" name="core_score_car_library_coach_total_weighted_score[{{$val1['template_section_id']}}][{{$val2->library_value_id}}][]" class="core_score_car_library_coach_total_weighted_score_{{$val2->library_value_id}}">
																	<tr>
																		<td>
