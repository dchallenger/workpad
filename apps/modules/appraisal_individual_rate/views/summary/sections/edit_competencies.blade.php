					...
					<tr class="q{{ $key+1 }}">
						...
						?>
											<td width="{{ $value->width }}">
												<input {{ $disabled }} type="text" question="{{ $val['library_value_id'] }}" value="{{ $self_comment_val }}" class="form-control {{ $value->class }}" name="field_appraisal[{{ $appraisee_user_id }}][{{ $section_id }}][{{ $library_id }}][{{ $val['library_value_id'] }}][]" data-inputmask="'alias': '{{ $value->data_type }}', 'autoGroup': true, 'groupSeparator': ',', 'groupSize': 3, 'repeat': 13, 'greedy' : false">
											</td>
						<?php
										break;
										case 3:
						?>
											<td width="{{ $value->width }}" rowspan="">
												<textarea {{ $disabled }} class="form-control {{ $value->class }}" rows="4" name="field_appraisal[{{ $appraisee_user_id }}][{{ $section_id }}][{{ $library_id }}][{{ $val['library_value_id'] }}][]">{{ $self_comment_val }}</textarea>
											</td>										
						<?php											
										break;
										case 9:
						?>
											<td width="{{ $value->width }}" rowspan="">
												<select class="form-control core_self_rating" ratio-weight="{{ $section_info->weight }}" name="field_appraisal[{{ $appraisee_user_id }}][{{ $section_id }}][{{ $library_id }}][{{ $val['library_value_id'] }}][]" section-id="{{ $section_info->template_section_id }}" question="{{ $val['library_value_id'] }}" {{$disabled}}>
