...
class Partners extends MY_PrivateController
{
	...
	public function detail($user_id=0, $child_call = false){
		...
		$data['organization'] = (count($organization) > 0 ? $organization[0]['key_value'] : "n/a");
		...
		$address_1 = (count($address_1) > 0 ? $address_1[0]['key_value'] : " ");
		...
		$data['emergency_relationship'] = (count($emergency_relationship) > 0 ? $emergency_relationship[0]['key_value'] : " ");
		$emergency_phone = $this->profile_mod->get_partners_personal($user_id, 'emergency_phone');
		$data['emergency_phone'] = (count($emergency_phone) > 0 ? $emergency_phone[0]['key_value'] : "n/a");
		$emergency_mobile = $this->profile_mod->get_partners_personal($user_id, 'emergency_mobile');
		$data['emergency_mobile'] = (count($emergency_mobile) > 0 ? $emergency_mobile[0]['key_value'] : "n/a");
		...
		$data['record']['health_care'] = (count($health_care) > 0 ? $health_care[0]['key_value'] : " ");
			...
			$data['record']['job_class'] = (count($job_class) > 0 ? $job_class[0]['key_value'] : " ");
			$employee_grade = $this->profile_mod->get_partners_personal($user_id, 'employee_grade');
			$data['record']['employee_grade'] = (count($employee_grade) > 0 ? $employee_grade[0]['key_value'] : " ");
		$job_rank_level = $this->profile_mod->get_partners_personal($user_id, 'job_rank_level');
        $data['record']['job_rank_level'] = (count($job_rank_level) > 0 ? $job_rank_level[0]['key_value'] : " ");
        $job_level = $this->profile_mod->get_partners_personal($user_id, 'job_level');
        $data['record']['job_level'] = (count($job_level) > 0 ? $job_level[0]['key_value'] : " ");
        $pay_level = $this->profile_mod->get_partners_personal($user_id, 'pay_level');
        $data['record']['pay_level'] = (count($pay_level) > 0 ? $pay_level[0]['key_value'] : " ");
		$pay_set_rates = $this->profile_mod->get_partners_personal($user_id, 'pay_set_rates');
        $data['record']['pay_set_rates'] = (count($pay_set_rates) > 0 ? $pay_set_rates[0]['key_value'] : " ");
        $competency_level = $this->profile_mod->get_partners_personal($user_id, 'competency_level');
        $data['record']['competency_level'] = (count($competency_level) > 0 ? $competency_level[0]['key_value'] : " ");
        $section = $this->profile_mod->get_partners_personal($user_id, 'section');
		$data['record']['section'] = (count($section) > 0 ? $section[0]['key_value'] : " ");
		/***** PERSONAL TAB *****/
		//Personal
		$gender = $this->profile_mod->get_partners_personal($user_id, 'gender');
		$data['gender'] = (count($gender) > 0 ? $gender[0]['key_value'] : " ");
		$birth_place = $this->profile_mod->get_partners_personal($user_id, 'birth_place');
		$data['birth_place'] = (count($birth_place) > 0 ? $birth_place[0]['key_value'] : " ");
...
/*			$validation_rules[] = 
			...
			case 2:
				...
				$partners_personal_key = array(
										'login_old' 	=> (isset($post['partners_personal']['old_id_number']) ? $post['partners_personal']['old_id_number'] : null)
										);

				$other_tables['users'] = $partners_personal_key;
				$partners_personal_key = array('old_id_number', 'probationary_date', 'last_probationary', 'last_salary_adjustment', 'organization',  'job_class', 'job_rank_level', 'job_level', 'pay_level', 'pay_set_rates', 'competency_level', 'employee_grade', 'section', 'home_leave');
				$partners_personal = $post['partners_personal'];
...
/*						if(CLIENT_DIR == 'riofil'){
							$1series = $this->db->get_where('system_series', array('series_code' => 'RIOFIL_ID_NUMBER'))->row();
						}
		...
		foreach( $other_tables as $table => $data )
		{
			...
			switch( true )
			{
				case $record->num_rows() == 0:
					...
					break;
				case $record->num_rows() == 1:
					$previous_other_data = $this->db->get_where($table, array($this->mod->primary_key => $this->record_id))->row_array();
				...
				foreach( $partners_personal_key as $table => $key )
				{
					...
					if(isset($partners_personal[$key]) && !is_array($partners_personal[$key])){
						if (isset($partners_personal[$key])) {
							...
							$this->mod->audit_logs($this->user->user_id, $this->mod->mod_code, $other_action, $table, $previous_other_data, $data_personal);
