...
class Audit_log extends MY_PrivateController
{
	...
	private function _get_list( $trash )
	{
		...
		if( is_array( $filter_by ) )
		{
			...
			{
				if ($filter_by == 'date_time') {
					$filter_value_tmp = explode('-', $filter_value);
					$filter = 'AND DATE('. $filter_by .') >= "'.date('Y-m-d',strtotime($filter_value_tmp[0])).'" AND DATE('.$filter_by .') <= "'.date('Y-m-d',strtotime($filter_value_tmp[1])).'"';
				} else
