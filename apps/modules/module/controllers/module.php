...
class Module extends MY_PrivateController
{
	...
	function save($child_call = false)
    {
        ...
        if( $this->response->saved )
        {
            ...
            if( !empty( $mod['modules.mod_code'] ) )
            {
                ...
                $permission = APPPATH . 'config/'.$this->session->userdata('current_db').' /profiles/1/permission.php';
                if( file_exists($permission) )
                {
                    unlink( $permission );
                    $this->load->model('profiles_model', 'profile');
                    $this->profile->_create_permission_file(1);
                }

                $permission = APPPATH . 'config/'.$this->session->userdata('current_db').' /roles/1/permission.php';
                if( file_exists($permission) )
                {
                    unlink( $permission );
                }                
            ...
            {    
                foreach($fs as $f_name => $field )
                {
                    if( $field['f_id'] == $f_id )
                    {
                        ...
                        if( $field['uitype_id'] == 4 || $field['uitype_id'] == 10 ){
