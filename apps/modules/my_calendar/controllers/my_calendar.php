...
/*        $data['focus_date']["val"] = date("F d, Y", $this->input->post('start_date'));
        ...
        $data['focus_date']["val"] = date("F d, Y", strtotime(date("F d, Y", $this->input->post('start_date')) . ' +1 day'));
        $data['date_from']["val"] = $date_from = date("F d, Y", strtotime(date("F d, Y", $this->input->post('start_date')) . ' +1 day'));
        $data['date_to']["val"] = $date_to = date("F d, Y", strtotime(date("F d, Y", $this->input->post('start_date')) . ' +1 day'));
        ...
        if(empty($forms_id)){
            ...
            if( in_array($data['form_id'], $leaves_ids) && $form_info['form_code'] != "ADDL" ){
                $already_exist = $this->mod->get_approved_forms(date("Y-m-d",strtotime($date_from)), $this->user->user_id);
            ...
            if(in_array($data['form_id'], $not_allowed_rest_holidays)){
                //check if restday
                $forms_rest_day = 0;
                $forms_rest_day = $this->mod->check_rest_day($this->user->user_id, date("Y-m-d",strtotime($date_from)));
                // $rest_day_not_allowed = 0;
                // foreach($forms_rest_day as $forms_rest){
                //     if(date("l", $date_from) == $forms_rest['rest_day']){
                ...
                //check if holiday
                $form_holiday = array();
                $form_holiday = $this->mod->check_if_holiday(date("Y-m-d",strtotime($date_from)), $this->user->user_id);
        ...
        switch($this->input->post('form_id')){
            ...
            case get_time_form_id('OBT'): //OBT
                //Get LEAVE BALANCE
                $data['shift_details'] = $this->mod->get_shift_details(date("Y-m-d", strtotime($date_from)), $this->user->user_id);
                $data['leave_balance'] = $this->formapp->get_leave_balance($this->user->user_id, date('Y-m-d'), 0);
                $data['date_from']["val"] = date("F d, Y", strtotime($date_from))." - ".date("h:i a", strtotime(isset($data['shift_details']['shift_time_start']) ? $data['shift_details']['shift_time_start'] : '00:00:00'));
                $data['date_to']["val"] = date("F d, Y", strtotime($date_from))." - ".date("h:i a", strtotime(isset($data['shift_details']['shift_time_end']) ? $data['shift_details']['shift_time_end'] : '00:00:00'));
                $data['location']["val"] = '';
            ...
            case get_time_form_id('OT'): //OT
                ...
                $data['shift_details'] = $this->mod->get_shift_details(date("Y-m-d", strtotime($date_from)), $this->user->user_id);
                if (empty($forms_id)){
                    $data['date_from']["val"] = date("F d, Y", strtotime($date_from))." - ".date("h:i a", strtotime(isset($data['shift_details']['shift_time_end']) ? $data['shift_details']['shift_time_end'] : '00:00:00'));
                    $data['date_to']["val"] = date("F d, Y", strtotime($date_from))." - ".date("h:i a", strtotime(isset($data['shift_details']['shift_time_end']) ? $data['shift_details']['shift_time_end'] : '00:00:00'));
                }
                break;
                ...
                $data['shift_details'] = $this->mod->get_shift_details(date("Y-m-d", strtotime($date_from)), $this->user->user_id);
                if (empty($forms_id)){
                    $data['date_from']["val"] = date("F d, Y", strtotime($date_from));
                    $data['date_to']["val"] = date("F d, Y", strtotime($date_from));  
                }          
                if( $forms_id ){
                    ...
                    if($data['ut_type']  == 0){
                        $logs_out = (isset($data['shift_details']['logs_time_out']) ? $data['shift_details']['logs_time_out'] : '-');
                        $data['ut_time_in_out'] = ($logs_out == "-") ? date("M d, Y - h:i A", strtotime(isset($data['shift_details']['shift_time_end']) ? $data['shift_details']['DATE'] .' '. $data['shift_details']['shift_time_end'] : '00:00:00')) : date("M d, Y - h:i A", strtotime($logs_out));
                    }else{
                        $logs_in = (isset($data['shift_details']['logs_time_in']) ? $data['shift_details']['logs_time_in'] : '-');
                        $data['ut_time_in_out'] = ($logs_in == "-") ? date("M d, Y - h:i A", strtotime(isset($data['shift_details']['shift_time_start']) ? $data['shift_details']['DATE'] .' '. $data['shift_details']['shift_time_start'] : '00:00:00')) : date("M d, Y - h:i A", strtotime($logs_in));
                    }
                }
            break;
            ...
            case get_time_form_id('DTRP'): //DTRP
                ...
                $data['shift_id']['val'] = (isset($data['shift_details']['shift_id']) ? $data['shift_details']['shift_id'] : 0);
                break;
            ...
            case get_time_form_id('RES'): //RES
                if( $forms_id ){
                    ...
                    $data['res_type'] = 1;
                    if (isset($data['shift_details']['logs_time_out'])) {
        ...
        $data['forms_title_date'] = date("F d, Y", strtotime($date_from)). " - ".date("D", strtotime($date_from));
        ...
        //$data['focus_date'] = date("Y-m-d", strtotime($date_from));
    ...
    function get_shift_details()
    {
        ...
        switch($this->input->post('type')){
            ...
            $shift_details['date_from'] = date("F d, Y", strtotime($date_to))." - ".date("h:i a", strtotime(isset($shift_details['shift_time_start']) ? $shift_details['shift_time_start'] : '00:00:00'));
            $shift_details['date_to'] = date("F d, Y", strtotime($date_from))." - ".date("h:i a", strtotime(isset($shift_details['shift_time_end']) ? $shift_details['shift_time_end'] : '00:00:00'));
            break;
            case 2: //OUT
            ...
            case 3: //IN-OUT
            $shift_details = $this->mod->get_shift_details( date('Y-m-d', strtotime($date_from)), $this->user->user_id);
            $shift_details['date_from'] = date("F d, Y", strtotime($date_from))." - ".date("h:i a", strtotime(isset($shift_details['shift_time_start']) ? $shift_details['shift_time_start'] : '00:00:00'));
            $shift_details['date_to'] = date("F d, Y", strtotime($date_to))." - ".date("h:i a", strtotime(isset($shift_details['shift_time_end']) ? $shift_details['shift_time_end'] : '00:00:00'));
            break;
        }

        $shift_end = (isset($shift_details['shift_time_end']) ? $shift_details['shift_time_end'] : '-');
        $shift_start = (isset($shift_details['shift_time_start']) ? $shift_details['shift_time_start'] : '-');
        $logs_out = (isset($shift_details['logs_time_out']) ? $shift_details['logs_time_out'] : '-');
        $logs_in = (isset($shift_details['logs_time_in']) ? $shift_details['logs_time_in'] : '-');
...
/*        debug($shift_details);
        ...
        if($this->input->post('form_type') == 10){ //undertime/excused tardiness form
            if($this->input->post('utype') == 0){
                $shift_details['ut_time_in_out'] = ($logs_out == "-") ? date("M d, Y - h:i A", strtotime(isset($shift_details['shift_time_end']) ? $shift_details['DATE'] .' '. $shift_details['shift_time_end'] : '00:00:00')) : date("M d, Y - h:i A", strtotime($logs_out));
            }else{
                $shift_details['ut_time_in_out'] = ($logs_in == "-") ? date("M d, Y - h:i A", strtotime(isset($shift_details['shift_time_start']) ? $shift_details['DATE'] .' '. $shift_details['shift_time_start'] : '00:00:00')) : date("M d, Y - h:i A", strtotime($logs_in));
            }
        }
    ...
    function save_form()
    {
        ...
        if($this->input->post('form_status_id') != 8){
            foreach($period as $dt) {
                ...
                $duration_credits = 0;
                $duration_total=0;
                if (isset($duration_date[$counter_duration])) {
            ...
            if( count($balance_data) > 0 ){
                if( $days <= $balance_data[0]['balance'] ) {
                    ...
                    foreach($time_forms_date_table as $datekey => $tfdates){
                        if($leavebal < 1){
                            $leavebalindex++;
                            if (isset($balance_data[$leavebalindex])) {
                                $leavebal = $balance_data[$leavebalindex]['balance'];
                            }
                        }
                        if (isset($balance_data[$leavebalindex])) {
