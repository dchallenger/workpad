                        ...
                        @if(count($financial_metric_planning['columns_arr']) > 0)
                            @foreach($financial_metric_planning['columns_arr'] as $key1 => $val1)
                                @if(array_key_exists($val1,$financial))
                                    ...
                                    <input type="hidden" name="financial_employee_kpi[{{$financial[$val1]->financial_metric_planning_details_id}}][{{$key1}}][target]" value="{{$financial[ $val1]->value}}">
                                    <input type="hidden" name="financial_employee_kpi[{{$financial[$val1]->financial_metric_planning_details_id}}][{{$key1}}][weight]" value="{{$financial[ $val1]->weight}}">
                                @else
                                    <td>&nbsp;</td>
                                @endif
                            @endforeach
