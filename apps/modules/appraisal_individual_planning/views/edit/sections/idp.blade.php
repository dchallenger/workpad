			...
			<tr>
				@foreach($columns->result_array() as $key => $val)
					<th width="{{ $val['width'] }}%" class="bold">
						...
						@if($key == 0)
							<span style="padding-left:5px"><a href="javasript:void(0)" class="btn-success" style="padding:1px 2px"><i class="fa fa-plus add_idp" data-section_id="{{ $section_id }}" data-scorecard_id="0" ></i></a></span>
						@endif						
					</th>
				@endforeach
				<th width="1%">
		...
		<tbody class="section-<?php echo $section_id ?> idp" section="<?php echo $section_id ?>">
					...
					<tr class="">
						...
						?>
											<td width="{{ $value->width }}" rowspan="">
												<select class="form-control" name="field[0][{{ $value->section_column_id }}][]" section-id="{{ $section_id }}">
						...
						?>
											<td width="{{ $value->width }}" rowspan="">
												<select class="form-control" ratio-weight="" name="field[0][{{ $value->section_column_id }}][]" section-id="{{ $section_id }}">
													<option value="0">Select</option>													
						...
						?>
											<td width="{{ $value->width }}" rowspan="">
												<textarea class="form-control" rows="2" name="field[0][{{ $value->section_column_id }}][]">{{ $planning_value }}</textarea>
											</td>										
						<?php											
										break;								
									}
			...
			@else
				<tr class="">
					...
					?>
										<td width="{{ $value->width }}" rowspan="">
											<select class="form-control" name="field[0][{{ $value->section_column_id }}][]" section-id="{{ $section_info->template_section_id }}">
												<option value="">Select</option>													
												@if($areas_development && $areas_development->num_rows() > 0)
													@foreach($areas_development->result() as $row)
					...
					?>
										<td width="{{ $value->width }}" rowspan="">
											<select class="form-control" ratio-weight="" name="field[0][{{ $value->section_column_id }}][]" section-id="{{ $section_info->template_section_id }}">
												<option value="0">Select</option>													
												@if($competencies && $competencies->num_rows() > 0)
													@foreach($competencies->result() as $row)
					...
					?>
										<td width="{{ $value->width }}" rowspan="">
											<textarea class="form-control" rows="2" name="field[0][{{ $value->section_column_id }}][]"></textarea>
										</td>										
					<?php											
									break;								
								}
