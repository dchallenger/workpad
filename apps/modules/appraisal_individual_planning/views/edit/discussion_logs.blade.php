    ...
    @if($planning->status_id != 4)
        <form id="discussion-form">
            <div class="chat-form margin-bottom-20 margin-top-0 @if($status_id=='-1') hidden @endif">
                ...
                <input name="approver_id" value="{{ $approver_id }}" type="hidden">
                ...
                <span class="pull-left small text-muted" style="margin-top: 10px;">Click Submit To to send comment.</span>
                <span class="pull-right" style="margin-top: 6px;"> 
                    <a href="javascript:submitDiscussion()" class="btn btn-sm green icn-only" > Submit To: {{$sent_to}}</a>
                </span>
            <?php endif;?>
            </div>
        </form>
