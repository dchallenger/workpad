...
class Hr_validation extends MY_PrivateController
{
    public function __construct()
    {
        $this->load->model('hr_validation_model', 'mod');
    ...
    public function index()
    {
        ...
        $data['permission_validation'] = isset($this->permission['list']) ? $this->permission['list'] : 0;
    ...
    function update_request()
    {
        ...
        if($_POST['action'] == 'approve'){
            ...
            $approver_record['forms_id'] = $record_id;
            $approver_record['user_id'] = $this->user->user_id;
            $approver_record['condition'] = 'By Level';
            $approver_record['sequence'] = 0;
            $approver_record['form_status_id'] = 6;
            $approver_record['comment'] = $validate_remarks;        
            $approver_record['comment_date'] = date('Y-m-d H:i:s');       
            $this->db->insert('time_forms_approver', $approver_record);

            // $sql = " UPDATE {$this->db->dbprefix}time_forms_approver 
            //         SET form_status_id = 2 
            //         WHERE forms_id = {$record_id} AND sequence = 1";
            // $this->db->query($sql);
        }else{
            $form_record['form_status_id'] = 9; //invalid
            $status_update = 'set to invalid.';
        ...
        $this->load->model('form_application_manage_model', 'app_manage');
        $forms_details          = $this->app_manage->get_form_information($record_id);

        $data['user_id']        = $this->session->userdata['user']->user_id;                                // THE CURRENT LOGGED IN USER 
        $data['display_name']   = $this->current_user['lastname']. ", ". $this->current_user['firstname'];  // THE CURRENT LOGGED IN USER'S DISPLAY NAME
        $data['feed_content']   = 'Your '.$forms_details['form'].
                                    ' was '.$status_update;                                                           // THE MAIN FEED BODY
        $data['recipient_id']   = $forms_details['user_id'];                                                               // TO WHOM THIS POST IS INTENDED TO
        $data['status']         = 'info';                                                                   // DANGER, INFO, SUCCESS & WARNING
        $data['message_type']   = 'Time Record';
        $data['forms_id']       = $record_id;

        $sp_time_forms_email_accounting = $this->db->query("CALL sp_time_forms_email_status({$record_id})");
        ...
        if($forms_details['is_leave'] == 1 && ($forms_details['with_credits'] == 1))
        {
            ...
            $time_forms_date_table = array();
            $time_forms_date_table = $this->db->get_where('time_forms_date', array($this->fam->primary_key => $forms_details['forms_id']))->result_array();
        ...
        if( $forms_details['form_code'] == 'ADDL' ){        
            $this->leave_convert();
        }

        $this->db->update( 'time_form_balance', array('modified_on' => date('Y-m-d H:i:s')), array('form_code' => $forms_details['form_code'], 'user_id' => $forms_details['user_id']) );
    ...
    {       
        ...
        $this->response->saved = true;
        // $this->response->message[] = array(
        //     'message' => "Additional Leave {$status_update}.",
        //     'type' => 'success'
        // );
