...
$config["list_cached_query"] = 'SELECT `ww_users_compensation`.`compensation_id` as record_id, 
	...
	ww_users_compensation.can_delete as "can_delete",
...
WHERE (
	IF(ww_users_compensation.status_id = 1, "Yes", "No") like "%{$search}%" OR 
	ww_users_compensation.description like "%{$search}%" OR 
	ww_users_compensation.compensation like "%{$search}%"
)';