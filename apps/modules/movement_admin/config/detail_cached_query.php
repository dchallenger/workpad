...
$config["detail_cached_query"] = 'SELECT `ww_partners_movement`.`movement_id` as record_id, 
	...
	ww_partners_movement_cause.cause as "partners_movement_due_to", 
...
LEFT JOIN `ww_users` T9 ON `T9`.`user_id` = `ww_partners_movement_approver_hr`.`user_id` AND sequence = 1
LEFT JOIN `ww_users` T10 ON `T10`.`user_id` = `ww_partners_movement_approver_hr`.`user_id` AND sequence = 2
LEFT JOIN `ww_users` T11 ON `T11`.`user_id` = `ww_partners_movement_approver_hr`.`user_id` AND sequence = 3
WHERE `ww_partners_movement`.`movement_id` = "{$record_id}" GROUP BY `ww_partners_movement`.`movement_id`';