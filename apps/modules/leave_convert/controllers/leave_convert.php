...
class Leave_convert extends MY_PrivateController
{
	public function __construct()
	{
		$this->load->model('leave_convert_model', 'mod');
    ...
    public function index()
    {
        ...
        $data['leave_convert'] = isset($this->permission['list']) ? $this->permission['list'] : 0;
    ...
    public function detail( $record_id, $child_call = false )
    {
        ...
        switch( $form_info['form_id'] ){
            ...
            case 27://addl
                $data['ot_leave'] = array();
                if($data['forms_id']){
                    $qry = "SELECT ot_leave.*, fstatus.form_status 
                            FROM {$this->db->dbprefix}time_forms_ot_leave ot_leave
                            INNER JOIN {$this->db->dbprefix}time_form_status fstatus
                            ON fstatus.form_status_id = ot_leave.status_id
                            WHERE ot_leave.{$this->mod->primary_key} = {$data['forms_id']} 
                            LIMIT 1";
                    $ot_leave = $this->db->query($qry);
                    if($ot_leave->num_rows() > 0){
                        $data['ot_leave'] = $ot_leave->row_array();
                    }
                }
                if( count($data['ot_leave']) == 0){
                    $data['leave_credit'] = $this->mod->get_conversion_processes($data['form_approver_details']);
                }
                $data['shift_details'] = $this->mod->get_shift_details(date("Y-m-d", strtotime($data['form_approver_details']['date_from'])), $forms_info['user_id']);
            break;
            case 10:
...
// exit();
    ...
    function update_request()
    {
        ...
        $transpo_record['remarks'] = $this->input->post('request_remarks');
        if($_POST['action'] == 'approve'){
            $transpo_record['status_id'] = 6;
            $status_update = 'successfully credited';
        }else{
            $transpo_record['status_id'] = 7;
            $status_update = 'was disapproved';
        }
