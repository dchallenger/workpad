...
class Performance_appraisal extends MY_PrivateController
{
	...
	public function save( $child_call = false )
	{
		...
		if( $this->response->saved )
		{
	        ...
	        // DELETE/INSERT 
	        if ($appraisal_status_id == 1) {
		        $applicable_for_data = $this->mod->getPerformancePlanningApplicable($planning_id);
		        $applicable_for_insert = array();
		        foreach($applicable_for_data->result_array() as $key => $value){
		        	$send_feeds = 0;
		        	
		            $template_id = $value['template_id'];
		            $performance_type_id = $value['performance_type_id'];
		            $planning_year = $value['year'];
					$performance_type = $this->db->get_where( 'performance_setup_performance' , array( 'performance_id' => $performance_type_id ) )->row_array();
		            $user_id = $value['user_id'];

					$full_name = $this->db->order_by('full_name', 'ASC')->get_where( 'users' , array( 'user_id' => $user_id ) )->row_array();

		            $applicable_for_insert[$this->mod->primary_key] = $this->response->record_id;
					$applicable_for_insert['template_id'] = $template_id;
					...
					$performance_populate_approvers = $this->db->query("CALL sp_performance_appraisal_populate_approvers({$this->response->record_id}, ".$user_id.", ".$user_id.")");
                    ...
                    $subject = $this->parser->parse_string($target_settings_send_template['subject'], $send_appraisal, TRUE); 

                    $this->db->query("INSERT INTO {$this->db->dbprefix}system_email_queue (`to`, `subject`, body)
                             VALUES('{$user_id}', '{$subject}', '".$this->db->escape_str($msg)."') ");					            

					$this->db->where($this->mod->primary_key,$this->response->record_id);
					$this->db->where('user_id',$user_id);							
					$this->db->update('performance_appraisal_applicable', array('email_sent' => 1, 'notification_sent' => 1));
	...
	// saving for abraham, justr rename and remove _abraham to work it back
	public function save_abraham( $child_call = false )
	{
		$this->_ajax_only();
		if( !$this->_set_record_id() )
		{
...
/*			$appraisal_get_approvers = $this->db->query("SELECT * 
	...
	public function get_planning(){
		...
		if(!empty($planning_details)){
			...
			if (isset($planning_details['performance_type_id'])) {
			...
			if (isset($planning_details['filter_by_id'])) {
			...
			if (isset($planning_details['template_id'])) {
				$query = "SELECT *
			...
			if (isset($planning_details['employment_status_id'])) {
