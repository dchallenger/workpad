		...
		<div>
	        <table width="100%" >
	            <?php
			        ...
			        $bonus_qry = $this->db->query($query." AND `group` = 'Earnings' AND `type` = 'bonus' AND user_id = ".$value->user_id.' ORDER BY transaction_label, transaction_class_code' );
			        if ($bonus_qry && $bonus_qry->num_rows() > 0)
			            $bonus = $bonus_qry->result();

			        $benefits_qry = $this->db->query($query." AND `group` = 'Earnings' AND `type` = 'benefits' AND user_id = ".$value->user_id.' ORDER BY transaction_label, transaction_class_code' );
			        if ($benefits_qry && $benefits_qry->num_rows() > 0)
			            $benefits = $benefits_qry->result();

			        $earning_qry = $this->db->query($query." AND `group` = 'Earnings' AND (`type` = 'Earnings' OR `type` = 'Bonus') AND user_id = ".$value->user_id.' ORDER BY transaction_label, transaction_class_code' );
			        if ($earning_qry && $earning_qry->num_rows() > 0)
			            $earning = $earning_qry->result();
	                ...
	                $ytd_qry = $this->db->query("SELECT IFNULL( ROUND( SUM( (   CASE 
	                                        ...
	                                        WHERE user_id = ".$value->user_id." AND payroll_date <= '".$value->payroll_date."' AND YEAR(payroll_date) = ".date("Y",strtotime($value->payroll_date)) );

					if ($ytd_qry && $ytd_qry->num_rows() > 0) {
						$ytd = $ytd_qry->row();
					}
