...
class MY_Lang extends MX_Lang {
  ...
  ***************************************************/
  ...
  function switch_uri($lang, $uri = "")
  {
    ...
    if ((!empty($uri)) && (array_key_exists($lang, $this->languages)))
    {
    ...
    if( !empty( $line ) )
    {
      if (strpos($line,'.') !== false || strpos($line,'/') !== false) {
        if (strpos($line,'.') !== false)
          $line_key = explode('.', $line);
        else
          $line_key = explode('/', $line);
        
        $value = !isset($this->language[$line_key[0]][$line_key[1]]) ? FALSE : $this->language[$line_key[0]][$line_key[1]];
      }
      else{
