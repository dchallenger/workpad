...
class forms_request_model extends Record
{
	var $mod_id;
	var $mod_code;
	var $route;
	...
	{
		$this->mod_id = 175;
		$this->mod_code = 'forms_request';
		$this->route = 'time/forms_request';
		$this->url = site_url('time/forms_request');
		$this->primary_key = 'forms_id';
		$this->table = 'time_forms';
		$this->icon = '';
		$this->short_name = 'With Request';
		$this->long_name  = 'With Request';
		$this->description = '';
		$this->path = APPPATH . 'modules/forms_request/';
...
function _get_list($start, $limit, $search, $filter, $trash = false)
{
	...
	$qry .= " WHERE time_forms_admin.form_code = 'OBT' AND time_forms_admin.time_forms_admin.form_status_id = 6 ";
...
function _get_list_cached_query()
{
	...
	return "SELECT time_forms_admin.*, obt_transpo.*, fstats.form_status as obt_status FROM `time_forms_admin` 
		INNER JOIN {$this->db->dbprefix}time_forms_obt_transpo obt_transpo 
