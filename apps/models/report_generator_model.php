	...
	function export_excel( $report, $columns, $result, $filter, $filter_var )
	{
		...
		switch($report->report_code)
		{
            ...
            case 'DAILY TIME RECORD':
				$dtrs_header = 1;
        		$excel = $this->load->view("templates/timekeeping_daily_time_record_summary_oclp", array('result' => $result,'filter' => $filter), true);
                break;
		...
		if (isset($tardy_header)) {
			...
			$content->getActiveSheet()->getStyle("A1")->applyFromArray($style_center);
		...
		if (isset($OTD_header)) {
			...
			$content->getActiveSheet()->getStyle("A1:M2")->applyFromArray($style_center);
			$content->getActiveSheet()->mergeCells('A1:M1');

			$content->getActiveSheet()->getStyle("A1:M2")->getFont()->setBold(true);

			$content->getActiveSheet()->getStyle("A2:M".($result->num_rows()+2))->applyFromArray($border_style);
		}
		...
		if (isset($appraisal_summary_header)) {
			$letters = $this->createColumnsArray('O');
			$index = 0;
			...
			$content->getActiveSheet()->getStyle("A1:O1")->getFont()->setBold(true);

			$content->getActiveSheet()->getStyle("A2:O".($result->num_rows()+1))->applyFromArray($border_style);
		}
    ...
    function export_txt( $report, $columns, $result, $posting_date = NULL, $filter )
    {
        ...
        switch($report->report_code)
        {
            ...
            case 'BIR1604CF 7.1':
                ...
                break;     
            case 'BIR1604CF 7.3':
                $txt = $this->load->view("templates/bir1604cf_7_3", array('columns' => $columns,'result' => $result, 'posting_date' => $posting_date, 'filter' => $filter), true);
                $fname = substr(str_replace('-', '', $res[0]['Comp Tin']),0,8).'.S73';
                break;   
            case 'BIR1604CF 7.4':
                $txt = $this->load->view("templates/bir1604cf_7_4", array('columns' => $columns,'result' => $result, 'posting_date' => $posting_date, 'filter' => $filter), true);
                $fname = substr(str_replace('-', '', $res[0]['Comp Tin']),0,8).'.S74';
                break;                                           
            case 'SSSTD':
            	$records = $result->row();
